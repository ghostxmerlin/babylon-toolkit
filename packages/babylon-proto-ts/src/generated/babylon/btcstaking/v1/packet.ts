 ...
 */
export interface ConsumerIBCPacket {
  slashedFp: SlashedFinalityProvider[];
}
...
function createBaseConsumerIBCPacket(): ConsumerIBCPacket {
  return { slashedFp: [] };
}

export const ConsumerIBCPacket: MessageFns<ConsumerIBCPacket> = {
  encode(message: ConsumerIBCPacket, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    for (const v of message.slashedFp) {
      SlashedFinalityProvider.encode(v!, writer.uint32(10).fork()).join();
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): ConsumerIBCPacket {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseConsumerIBCPacket();
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1: {
          ...
          message.slashedFp.push(SlashedFinalityProvider.decode(reader, reader.uint32()));
          continue;
        }
      }
      if ((tag & 7) === 4 || tag === 0) {
  ...
  fromJSON(object: any): ConsumerIBCPacket {
    return {
      slashedFp: globalThis.Array.isArray(object?.slashedFp)
        ? object.slashedFp.map((e: any) => SlashedFinalityProvider.fromJSON(e))
        : [],
    };
  },

  toJSON(message: ConsumerIBCPacket): unknown {
    const obj: any = {};
    if (message.slashedFp?.length) {
      obj.slashedFp = message.slashedFp.map((e) => SlashedFinalityProvider.toJSON(e));
    }
    return obj;
  },

  create<I extends Exact<DeepPartial<ConsumerIBCPacket>, I>>(base?: I): ConsumerIBCPacket {
    return ConsumerIBCPacket.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<ConsumerIBCPacket>, I>>(object: I): ConsumerIBCPacket {
    const message = createBaseConsumerIBCPacket();
    message.slashedFp = object.slashedFp?.map((e) => SlashedFinalityProvider.fromPartial(e)) || [];
    return message;
  },
};
...
function createBaseNewFinalityProvider(): NewFinalityProvider {
  return { description: undefined, commission: "", addr: "", btcPkHex: "", pop: undefined, consumerId: "" };
}
...
export const NewFinalityProvider: MessageFns<NewFinalityProvider> = {
  encode(message: NewFinalityProvider, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    if (message.consumerId !== "") {
      writer.uint32(66).string(message.consumerId);
    }
    return writer;
  },
