...
export const Minter: MessageFns<Minter> = {
  encode(message: Minter, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    if (message.previousBlockTime !== undefined) {
      Timestamp.encode(toTimestamp(message.previousBlockTime), writer.uint32(34).fork()).join();
    }
...
export const GenesisTime: MessageFns<GenesisTime> = {
  encode(message: GenesisTime, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): GenesisTime {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
